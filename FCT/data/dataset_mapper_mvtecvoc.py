...
class DatasetMapperWithSupportMVTECVOC:
        ...
        if self.is_train:
            ...
            if self.few_shot:
                ...
                for idx, dataset_name in enumerate(cfg.DATASETS.TRAIN):
                    ...
                    support_df_tmp = pd.read_pickle(
                        "./datasets/mvtecvoc/{}.pkl".format(dataset_name)
                    )
                    print(
                        "training support_df=",
                        "./datasets/mvtecvoc/{}.pkl".format(dataset_name),
                    ...
                    if idx == 0:
                        ...
                        self.support_df = self.support_df.append(
                            support_df_tmp, ignore_index=True
                        )
    ...
    def __call__(self, dataset_dict):
        ...
        if self.is_train:
            ...
            if self.support_on:
                ...
                support_images, support_bboxes, support_cls = self.generate_support(
                    dataset_dict
                )
                dataset_dict["support_images"] = torch.as_tensor(
                    np.ascontiguousarray(support_images)
                )
    ...
    def generate_support(self, dataset_dict):
        ...
        all_cls = self.support_df.loc[
            self.support_df["image_id"] == query_img, "category_id"
        ].tolist()
        ...
        support_data_all = np.zeros(
            (support_way * support_shot, 3, 320, 320), dtype=np.float32
        )
        ...
        for shot in range(support_shot):
            ...
            support_cls = self.support_df.loc[
                self.support_df["id"] == support_id, "category_id"
            ].tolist()[0]
            support_img = self.support_df.loc[
                self.support_df["id"] == support_id, "image_id"
            ].tolist()[0]
        ...
        if support_way == 1:
            ...
            for way in range(support_way - 1):
                other_cls = (
                    self.support_df.loc[
                        (~self.support_df["category_id"].isin(used_category_id)),
                        "category_id",
                    ]
                    .drop_duplicates()
                    .sample()
                ...
                for shot in range(support_shot):
                    ...
                    support_list = self.support_df.loc[
                        (self.support_df["category_id"] == other_cls)
                        & (~self.support_df["image_id"].isin(used_image_id))
                        & (~self.support_df["id"].isin(used_id_ls)),
                        "id",
                    ]
                    ...
                    support_db = self.support_df.loc[
                        self.support_df["id"] == support_id, :
                    ]
                    assert support_db["id"].values[0] == support_id
